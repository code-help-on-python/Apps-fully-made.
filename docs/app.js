...
// Allowlist: hostname -> required path prefix (use "/" for any path)
const LICENSE_ALLOWLIST = {
  // GitHub Pages (served under /Crypto-tool)
  "code-help-on-python.github.io": "/Crypto-tool",

  // Netlify (served at root)
...
function isLicensedOrigin() {
  ...
  // Allow local file usage (opened directly from disk)
  ...
  const isNetlifyPreview = host.endsWith(".netlify.app") && host.includes("cryptoshield-tool");
